...
uses
  ...
  StdApp.Config,
  CPaskal.Common,
  CPaskal.Debug.Server,
  CPaskal.Debug.Client,
  CPaskal.ZigBuild;

const
  CP_DBG_DEFAULT_DAP_PORT = 4711;
  CP_DBG_TIMOUT_CONTINUE_MS = 5000;
  CP_DBG_TIMEOUT_MS = 2000;

type
  { TCPREPLBreakpoint }
  TCPREPLBreakpoint = record
    SourceFile: string;
    SourceLine: Integer;
    Verified: Boolean;
  end;
  ...
  { TCPDebugREPL }
  TCPDebugREPL = class(TBaseObject)
  private
    FServer: TCPDebugServer;
    FClient: TCPDebugClient;
    FServerThread: TThread;
    FBreakpoints: TList<TCPREPLBreakpoint>;
    FRunning: Boolean;
    FExePath: string;
    FPort: Integer;
    FPrompt: string;
  ...
  public
    ...
    procedure Run(const AExePath: string; const APort: Integer = CP_DBG_DEFAULT_DAP_PORT);
    procedure Stop();
...
{ TCPDebugREPL }
constructor TCPDebugREPL.Create();
begin
  inherited Create();
  FServer := nil;
  FClient := nil;
  FServerThread := nil;
  FBreakpoints := TList<TCPREPLBreakpoint>.Create();
  FRunning := False;
  FExePath := '';
  FPort := CP_DBG_DEFAULT_DAP_PORT;
  FPrompt := '(dbg) ';
  FTimeoutContinueMS := CP_DBG_TIMOUT_CONTINUE_MS;
  FTimeoutStepMS := CP_DBG_TIMEOUT_MS;
end;

destructor TCPDebugREPL.Destroy();
begin
  StopSession();
  FBreakpoints.Free();
  inherited Destroy();
...
procedure TCPDebugREPL.StopSession();
begin
...
procedure TCPDebugREPL.StartSession();
var
  LExePath: string;
  LPort: Integer;
begin
  ...
  FClient := TCPDebugClient.Create();
end;

function TCPDebugREPL.DoDAHandshake(): Boolean;
begin
  Result := False;
...
procedure TCPDebugREPL.ShowSourceContext();
var
  LFrames: TArray<TCPDAPClientStackFrame>;
  LSourceFile: string;
  LSourceLine: Integer;
  LLines: TStringList;
  LStart: Integer;
...
procedure TCPDebugREPL.SendBreakpointsForFile(const ASourceFile: string);
var
  LLines: TList<Integer>;
  LI: Integer;
  LBP: TCPREPLBreakpoint;
begin
...
procedure TCPDebugREPL.LoadBreakpointsFromFile(const APath: string);
var
  LConfig: TConfig;
  LCount: Integer;
  LI: Integer;
...
procedure TCPDebugREPL.Run(const AExePath: string; const APort: Integer);
var
  LCommand: string;
...
begin
  ...
  TConsole.PrintLn('');
  TConsole.PrintLn(COLOR_CYAN + '=== CPaskal Debug REPL ===' + COLOR_RESET);
  TConsole.PrintLn('Executable: %s', [TUtils.ResolvePath(FExePath)]);
  TConsole.PrintLn('');
  ...
  LBreakpointFile := TUtils.ResolvePath(TPath.ChangeExtension(FExePath, CP_BREAKPOINT_EXT));
...
procedure TCPDebugREPL.ProcessCommand(const ACommand: string);
begin
  if ACommand = '' then
    Exit;
...
procedure TCPDebugREPL.ShowHelp();
begin
...
procedure TCPDebugREPL.HandleSetBreakpoint(const ACommand: string);
var
  LColonPos: Integer;
  LFile: string;
  LLine: Integer;
...
procedure TCPDebugREPL.HandleListBreakpoints();
var
  LI: Integer;
  LBP: TCPREPLBreakpoint;
begin
...
procedure TCPDebugREPL.HandleDeleteBreakpoint(const ACommand: string);
var
  LIndex: Integer;
  LBP: TCPREPLBreakpoint;
  LFile: string;
...
procedure TCPDebugREPL.HandleClearBreakpoints();
var
  LFiles: TDictionary<string, Boolean>;
  LBP: TCPREPLBreakpoint;
  LKey: string;
...
procedure TCPDebugREPL.HandleBacktrace();
var
  LFrames: TArray<TCPDAPClientStackFrame>;
  LI: Integer;
begin
...
procedure TCPDebugREPL.HandleLocals();
var
  LFrames: TArray<TCPDAPClientStackFrame>;
  LScopes: TArray<TCPDAPClientScope>;
  LVars: TArray<TCPDAPClientVariable>;
  LI: Integer;
  LJ: Integer;
begin
  if FClient.State <> dcsStopped then
...
procedure TCPDebugREPL.HandlePrint(const ACommand: string);
var
  LExpr: string;
  LResult: string;
begin
...
procedure TCPDebugREPL.HandleContinue();
begin
  if FClient.State <> dcsStopped then
  begin
    TConsole.PrintLn(COLOR_RED + 'Not stopped' + COLOR_RESET);
...
procedure TCPDebugREPL.HandleNext();
begin
  if FClient.State <> dcsStopped then
  begin
    TConsole.PrintLn(COLOR_RED + 'Not stopped' + COLOR_RESET);
...
procedure TCPDebugREPL.HandleStepInto();
begin
  if FClient.State <> dcsStopped then
  begin
    TConsole.PrintLn(COLOR_RED + 'Not stopped' + COLOR_RESET);
...
procedure TCPDebugREPL.HandleStepOut();
begin
  if FClient.State <> dcsStopped then
  begin
    TConsole.PrintLn(COLOR_RED + 'Not stopped' + COLOR_RESET);
...
procedure TCPDebugREPL.HandleRestart();
var
  LSavedBreakpoints: TList<TCPREPLBreakpoint>;
  LBP: TCPREPLBreakpoint;
  LFiles: TDictionary<string, Boolean>;
  LKey: string;
begin
  ...
  LSavedBreakpoints := TList<TCPREPLBreakpoint>.Create();
  try
    for LBP in FBreakpoints do
...
procedure TCPDebugREPL.HandleFile(const ACommand: string);
var
  LPath: string;
begin
...
procedure TCPDebugREPL.HandleVerbose(const ACommand: string);
begin
...
procedure TCPDebugREPL.HandleThreads();
begin
