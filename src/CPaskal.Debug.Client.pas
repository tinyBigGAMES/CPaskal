...
type
  ...
  { TCPDAPClientVariable }
  TCPDAPClientVariable = record
    VarName: string;
    VarValue: string;
    VarType: string;
  ...
  { TCPDAPClientStackFrame }
  TCPDAPClientStackFrame = record
    FrameID: Integer;
    FunctionName: string;
    SourceFile: string;
    SourceLine: Integer;
  ...
  { Callback types }
  TCPDAPClientStoppedCallback = reference to procedure(const AReason: string; const AThreadId: Integer);
  TCPDAPClientOutputCallback = reference to procedure(const AOutput: string);
  TCPDAPClientExitedCallback = reference to procedure(const AExitCode: Integer);

  { TCPDebugClient }
  TCPDebugClient = class(TBaseObject)
  private
    FSocket: TSocket;
    FState: TCPDAPClientState;
    FNextSeq: Integer;
    FLastError: string;
    ...
    // Callbacks
    FOnStopped: TCPDAPClientStoppedCallback;
    FOnOutput: TCPDAPClientOutputCallback;
    FOnExited: TCPDAPClientExitedCallback;
  ...
  public
    ...
    // Inspection
    function GetCallStack(const AThreadId: Integer = 0): TArray<TCPDAPClientStackFrame>;
    function GetScopes(const AFrameId: Integer): TArray<TCPDAPClientScope>;
    function GetVariables(const AVariablesReference: Integer): TArray<TCPDAPClientVariable>;
    ...
    function GetState(): TCPDAPClientState;
    function GetCurrentThreadId(): Integer;

    // Properties
    property State: TCPDAPClientState read FState;
    property VerboseLogging: Boolean read FVerboseLogging write FVerboseLogging;
    property OnStopped: TCPDAPClientStoppedCallback read FOnStopped write FOnStopped;
    property OnOutput: TCPDAPClientOutputCallback read FOnOutput write FOnOutput;
    property OnExited: TCPDAPClientExitedCallback read FOnExited write FOnExited;
  end;

implementation

{ TCPDebugClient }
constructor TCPDebugClient.Create();
begin
  inherited Create();
...
destructor TCPDebugClient.Destroy();
begin
  Disconnect();
  inherited Destroy();
end;

function TCPDebugClient.GetNextSeq(): Integer;
begin
  Result := FNextSeq;
  Inc(FNextSeq);
end;

procedure TCPDebugClient.SetError(const AError: string);
begin
  FLastError := AError;
end;

function TCPDebugClient.HasError(): Boolean;
begin
  Result := FLastError <> '';
end;

function TCPDebugClient.GetLastError(): string;
begin
  Result := FLastError;
end;

function TCPDebugClient.GetState(): TCPDAPClientState;
begin
  Result := FState;
end;

function TCPDebugClient.GetCurrentThreadId(): Integer;
begin
  Result := FCurrentThreadId;
end;

function TCPDebugClient.Connect(const AHost: string; const APort: Integer): Boolean;
var
  LWSAData: TWSAData;
  LAddr: TSockAddrIn;
  LHostEnt: PHostEnt;
...
procedure TCPDebugClient.Disconnect();
begin
  if FSocket <> INVALID_SOCKET then
  begin
    closesocket(FSocket);
...
function TCPDebugClient.SendRaw(const AData: string): Boolean;
var
  LBytes: TBytes;
  LSent: Integer;
  LTotal: Integer;
...
procedure TCPDebugClient.SendDAPMessage(const AJson: string);
var
  LMsg: string;
begin
...
function TCPDebugClient.ReadDAPMessage(out AJson: string;
...
function TCPDebugClient.SendRequest(const ACommand: string;
...
procedure TCPDebugClient.ProcessEvent(const AEvent: TJSONObject);
var
  LEventName: string;
  LBody: TJSONObject;
  LReason: string;
...
procedure TCPDebugClient.ProcessPendingEvents(const ATimeoutMS: Integer);
var
  LJson: string;
  LObj: TJSONObject;
  LMsgType: string;
...
function TCPDebugClient.Initialize(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
...
function TCPDebugClient.Launch(const AProgram: string;
...
function TCPDebugClient.ConfigurationDone(): Boolean;
var
  LResponse: TJSONObject;
begin
  Result := False;
...
function TCPDebugClient.SetBreakpoints(const ASourcePath: string;
...
function TCPDebugClient.DoContinue(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
...
function TCPDebugClient.StepOver(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
...
function TCPDebugClient.StepIn(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
...
function TCPDebugClient.StepOut(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
...
function TCPDebugClient.GetCallStack(const AThreadId: Integer): TArray<TCPDAPClientStackFrame>;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
  LBody: TJSONObject;
...
function TCPDebugClient.GetScopes(const AFrameId: Integer): TArray<TCPDAPClientScope>;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
  LBody: TJSONObject;
...
function TCPDebugClient.GetVariables(const AVariablesReference: Integer): TArray<TCPDAPClientVariable>;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
  LBody: TJSONObject;
...
function TCPDebugClient.Evaluate(const AExpression: string;
...
function TCPDebugClient.DisconnectDAP(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
