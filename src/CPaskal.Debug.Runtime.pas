...
type
  ...
  { TCPBreakpointManager }
  TCPBreakpointManager = class(TBaseObject)
  private
    FTarget: TCPDebugTarget;         // Reference (not owned)
    FSourceMap: TCPPDBSourceMap;     // Reference (not owned)
    FBreakpoints: TList<TCPBreakpointInfo>;
    FNextID: Integer;
  ...
  public
    ...
    procedure SetTarget(const ATarget: TCPDebugTarget);
    procedure SetSourceMap(const ASourceMap: TCPPDBSourceMap);
    ...
    function GetBreakpointAt(const AAddress: UInt64;
      out AInfo: TCPBreakpointInfo): Boolean;
    function GetBreakpointByID(const AID: Integer;
      out AInfo: TCPBreakpointInfo): Boolean;
    function GetBreakpointCount(): Integer;
    function GetBreakpoint(const AIndex: Integer): TCPBreakpointInfo;
  ...
  { TCPStackWalker }
  TCPStackWalker = class(TBaseObject)
  public
    function WalkStack(const ATarget: TCPDebugTarget;
      const ASourceMap: TCPPDBSourceMap;
      const AContext: TContext): TArray<TCPDebugStackFrame>;
  end;

  { TCPDebugRuntime }
  TCPDebugRuntime = class(TBaseObject)
  private
    FTarget: TCPDebugTarget;              // Reference (not owned)
    FSourceMap: TCPPDBSourceMap;           // Reference (not owned)
    FBreakpoints: TCPBreakpointManager;   // Owned
    FStackWalker: TCPStackWalker;         // Owned
    FLastStopEvent: TCPDebugStopEvent;
  ...
  public
    ...
    // Inspection
    function GetCallStack(): TArray<TCPDebugStackFrame>;
    function GetLastStopEvent(): TCPDebugStopEvent;
    function GetVariables(): TArray<TCPDebugVariable>;
    function Evaluate(const AExpression: string): TCPDebugVariable;

    // Access
    function GetBreakpoints(): TCPBreakpointManager;
    function GetTarget(): TCPDebugTarget;
...
{ TCPBreakpointManager }
constructor TCPBreakpointManager.Create();
begin
  inherited Create();
  FTarget := nil;
  FSourceMap := nil;
  FBreakpoints := TList<TCPBreakpointInfo>.Create();
  FNextID := 1;
end;

destructor TCPBreakpointManager.Destroy();
begin
  RemoveAll();
  FBreakpoints.Free();
  inherited Destroy();
...
procedure TCPBreakpointManager.SetTarget(const ATarget: TCPDebugTarget);
begin
  FTarget := ATarget;
end;

procedure TCPBreakpointManager.SetSourceMap(const ASourceMap: TCPPDBSourceMap);
begin
  FSourceMap := ASourceMap;
end;

function TCPBreakpointManager.SetBreakpoint(const AFile: string;
  const ALine: Integer; const ACondition: string;
  const AHitCondition: Integer): Integer;
var
  LInfo: TCPBreakpointInfo;
  LAddress: UInt64;
begin
  Result := -1;
...
function TCPBreakpointManager.RemoveBreakpoint(const AID: Integer): Boolean;
var
  LI: Integer;
begin
  Result := False;
...
function TCPBreakpointManager.SetTempBreakpoint(const AAddress: UInt64): Integer;
var
  LInfo: TCPBreakpointInfo;
begin
  if FTarget = nil then
...
procedure TCPBreakpointManager.RemoveAllTemp();
var
  LI: Integer;
begin
...
procedure TCPBreakpointManager.RemoveAll();
var
  LI: Integer;
begin
  for LI := FBreakpoints.Count - 1 downto 0 do
...
function TCPBreakpointManager.IsOurBreakpoint(const AAddress: UInt64): Boolean;
var
  LI: Integer;
begin
  Result := False;
...
function TCPBreakpointManager.GetBreakpointAt(const AAddress: UInt64;
  out AInfo: TCPBreakpointInfo): Boolean;
var
  LI: Integer;
begin
...
function TCPBreakpointManager.GetBreakpointByID(const AID: Integer;
  out AInfo: TCPBreakpointInfo): Boolean;
var
  LI: Integer;
begin
...
function TCPBreakpointManager.GetBreakpointCount(): Integer;
begin
  Result := FBreakpoints.Count;
end;

function TCPBreakpointManager.GetBreakpoint(const AIndex: Integer): TCPBreakpointInfo;
begin
  Result := FBreakpoints[AIndex];
end;

procedure TCPBreakpointManager.ApplyAll();
var
  LI: Integer;
  LInfo: TCPBreakpointInfo;
  LAddress: UInt64;
...
procedure TCPBreakpointManager.PatchBreakpoint(const AIndex: Integer);
var
  LInfo: TCPBreakpointInfo;
begin
  ...
  FTarget.WriteByte(LInfo.Address, CP_DBG_INT3_OPCODE);
...
procedure TCPBreakpointManager.UnpatchBreakpoint(const AIndex: Integer);
var
  LInfo: TCPBreakpointInfo;
begin
  LInfo := FBreakpoints[AIndex];
...
function TCPStackWalker.WalkStack(const ATarget: TCPDebugTarget;
  const ASourceMap: TCPPDBSourceMap;
  const AContext: TContext): TArray<TCPDebugStackFrame>;
var
  LFrame: TCPDebugStackFrame;
  LFrameID: Integer;
  LFile: string;
  LLine: Integer;
  LFuncName: string;
...
{ TCPDebugRuntime }
constructor TCPDebugRuntime.Create();
begin
  inherited Create();
  FTarget := nil;
  FSourceMap := nil;
  FBreakpoints := TCPBreakpointManager.Create();
  FStackWalker := TCPStackWalker.Create();
  FillChar(FLastStopEvent, SizeOf(FLastStopEvent), 0);
end;

destructor TCPDebugRuntime.Destroy();
begin
  FStackWalker.Free();
  FBreakpoints.Free();
  inherited Destroy();
...
procedure TCPDebugRuntime.SetTarget(const ATarget: TCPDebugTarget);
begin
  FTarget := ATarget;
  FBreakpoints.SetTarget(ATarget);
end;

procedure TCPDebugRuntime.SetSourceMap(const ASourceMap: TCPPDBSourceMap);
begin
  FSourceMap := ASourceMap;
  FBreakpoints.SetSourceMap(ASourceMap);
end;

function TCPDebugRuntime.SetBreakpoint(const AFile: string;
  const ALine: Integer; const ACondition: string;
  const AHitCondition: Integer): Integer;
begin
...
function TCPDebugRuntime.RemoveBreakpoint(const AID: Integer): Boolean;
begin
  Result := FBreakpoints.RemoveBreakpoint(AID);
end;

procedure TCPDebugRuntime.ConfigurationDone();
begin
...
function TCPDebugRuntime.DoContinue(const ASetTrapFlag: Boolean): Boolean;
var
  LBPInfo: TCPBreakpointInfo;
  LContext: TContext;
  LAddress: UInt64;
begin
  Result := False;
...
function TCPDebugRuntime.StepOver(): Boolean;
var
  LContext: TContext;
  LCurrentFile: string;
...
function TCPDebugRuntime.StepIn(): Boolean;
var
  LContext: TContext;
  LAddr: UInt64;
  LOpcode: Byte;
...
function TCPDebugRuntime.StepOut(): Boolean;
var
  LContext: TContext;
  LReturnAddr: UInt64;
  LOpcode: Byte;
...
function TCPDebugRuntime.WaitForStop(): Boolean;
var
  LEvent: TCPDebugStopEvent;
  LBPInfo: TCPBreakpointInfo;
  LI: Integer;
  LFound: Boolean;
  LShouldBreak: Boolean;
  LCondVar: TCPDebugVariable;
begin
  Result := False;
  if FTarget = nil then
    Exit;
...
function TCPDebugRuntime.GetCallStack(): TArray<TCPDebugStackFrame>;
var
  LContext: TContext;
begin
  if (FTarget = nil) or (FSourceMap = nil) then
...
function TCPDebugRuntime.GetLastStopEvent(): TCPDebugStopEvent;
begin
  Result := FLastStopEvent;
end;

function TCPDebugRuntime.GetVariables(): TArray<TCPDebugVariable>;
const
  CV_AMD64_RBP = 334;
  CV_AMD64_RSP = 335;
var
  LVars: TArray<TCPDebugVariable>;
  LContext: TContext;
  LAddress: UInt64;
  LRegBase: UInt64;
  LBytes: TBytes;
  LI: Integer;
  LVar: TCPDebugVariable;
  LResultList: TList<TCPDebugVariable>;
begin
  Result := nil;
  if (FTarget = nil) or (FSourceMap = nil) then
    Exit;
  ...
  LResultList := TList<TCPDebugVariable>.Create();
  try
...
function TCPDebugRuntime.Evaluate(const AExpression: string): TCPDebugVariable;
const
  CV_AMD64_RSP = 335;
var
  LVars: TArray<TCPDebugVariable>;
  ...
  function ReadVarValue(var AVar: TCPDebugVariable): Boolean;
  begin
    Result := False;
    AVar.VarValue := '???';
    if AVar.Size = 0 then
...
begin
  Result := Default(TCPDebugVariable);
...
function TCPDebugRuntime.GetBreakpoints(): TCPBreakpointManager;
begin
  Result := FBreakpoints;
end;

function TCPDebugRuntime.GetTarget(): TCPDebugTarget;
begin
  Result := FTarget;
end;

function TCPDebugRuntime.GetSourceMap(): TCPPDBSourceMap;
begin
  Result := FSourceMap;
end;
