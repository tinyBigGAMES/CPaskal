...
type
  ...
  { TCPDAPServer }
  TCPDAPServer = class(TBaseObject)
  private
    FRuntime: TCPDebugRuntime;        // Reference (not owned)
  ...
  public
    ...
    procedure SetRuntime(const ARuntime: TCPDebugRuntime);
    ...
    function GetState(): TCPDAPState;
  end;

implementation

{ TCPDAPServer }
constructor TCPDAPServer.Create();
begin
  inherited Create();
  FRuntime := nil;
  FSourceMap := nil;
...
destructor TCPDAPServer.Destroy();
begin
  StopServer();
  inherited Destroy();
end;

procedure TCPDAPServer.SetRuntime(const ARuntime: TCPDebugRuntime);
begin
  FRuntime := ARuntime;
end;

procedure TCPDAPServer.SetSourceMap(const ASourceMap: TCPPDBSourceMap);
begin
  FSourceMap := ASourceMap;
end;

function TCPDAPServer.GetPort(): Integer;
begin
  Result := FPort;
end;

function TCPDAPServer.GetState(): TCPDAPState;
begin
  Result := FState;
end;

function TCPDAPServer.StartListening(const APort: Integer): Boolean;
var
  LAddr: TSockAddrIn;
  LOptVal: Integer;
begin
...
function TCPDAPServer.WaitForConnection(): Boolean;
begin
  Result := False;
...
procedure TCPDAPServer.StopServer();
begin
  if FClientSocket <> INVALID_SOCKET then
  begin
...
function TCPDAPServer.ReadMessage(): TJSONObject;
var
  LBuf: AnsiChar;
  LHeader: AnsiString;
...
procedure TCPDAPServer.SendMessage(const AMsg: TJSONObject);
var
  LBody: TBytes;
  LHeader: AnsiString;
begin
...
procedure TCPDAPServer.SendResponse(const ARequestSeq: Integer;
  const ACommand: string; const ASuccess: Boolean;
...
procedure TCPDAPServer.SendEvent(const AEventName: string;
...
procedure TCPDAPServer.RunMessageLoop();
var
  LMsg: TJSONObject;
  LCommand: string;
  LSeq: Integer;
...
procedure TCPDAPServer.DispatchRequest(const AMsg: TJSONObject);
var
  LCommand: string;
  LSeq: Integer;
  LArgs: TJSONObject;
...
procedure TCPDAPServer.HandleInitialize(const ASeq: Integer;
  const AArgs: TJSONObject);
var
  LBody: TJSONObject;
  LFilters: TJSONArray;
...
procedure TCPDAPServer.HandleLaunch(const ASeq: Integer;
  const AArgs: TJSONObject);
begin
...
procedure TCPDAPServer.HandleSetBreakpoints(const ASeq: Integer;
  const AArgs: TJSONObject);
var
  LSource: TJSONObject;
  LSourcePath: string;
...
procedure TCPDAPServer.HandleSetExceptionBreakpoints(const ASeq: Integer;
  const AArgs: TJSONObject);
var
  LFilters: TJSONArray;
  LI: Integer;
  LEnabled: Boolean;
  LTarget: TCPDebugTarget;
begin
  LEnabled := False;
...
procedure TCPDAPServer.HandleConfigurationDone(const ASeq: Integer);
begin
...
procedure TCPDAPServer.HandleThreads(const ASeq: Integer);
var
  LBody: TJSONObject;
  LThreads: TJSONArray;
  LThread: TJSONObject;
...
procedure TCPDAPServer.HandleStackTrace(const ASeq: Integer;
  const AArgs: TJSONObject);
var
  LBody: TJSONObject;
  LFrames: TJSONArray;
  ...
  LStack: TArray<TCPDebugStackFrame>;
  LI: Integer;
begin
...
procedure TCPDAPServer.HandleScopes(const ASeq: Integer;
  const AArgs: TJSONObject);
var
  LBody: TJSONObject;
  LScopes: TJSONArray;
...
procedure TCPDAPServer.HandleVariables(const ASeq: Integer;
  const AArgs: TJSONObject);
var
  LBody: TJSONObject;
...
procedure TCPDAPServer.HandleContinue(const ASeq: Integer);
var
  LBody: TJSONObject;
begin
...
procedure TCPDAPServer.HandleNext(const ASeq: Integer);
begin
  if FState <> dsStopped then
  begin
...
procedure TCPDAPServer.HandleStepIn(const ASeq: Integer);
begin
  if FState <> dsStopped then
  begin
...
procedure TCPDAPServer.HandleStepOut(const ASeq: Integer);
begin
  if FState <> dsStopped then
  begin
...
procedure TCPDAPServer.HandlePause(const ASeq: Integer);
begin
  // Not fully supported in v1 -- would need to inject INT3
  SendResponse(ASeq, 'pause', True);
end;

procedure TCPDAPServer.HandleEvaluate(const ASeq: Integer;
  const AArgs: TJSONObject);
var
  LExpression: string;
  LBody: TJSONObject;
...
procedure TCPDAPServer.HandleDisconnect(const ASeq: Integer);
begin
...
procedure TCPDAPServer.ProcessStopEvent();
var
  LBody: TJSONObject;
