...
const
  ...
  CP_DBG_TRAP_FLAG = $100;

  // INT3 opcode
  CP_DBG_INT3_OPCODE = $CC;
...
type
  ...
  { TCPDebugStopEvent }
  TCPDebugStopEvent = record
    Reason: TCPDebugStopReason;
  ...
  { TCPDebugTarget }
  TCPDebugTarget = class(TBaseObject)
  protected
  ...
  public
    ...
    function WaitForStop(out AEvent: TCPDebugStopEvent): Boolean; virtual; abstract;
  ...
  { TCPPEDebugTarget }
  TCPPEDebugTarget = class;

  { TCPPEDebugLoopThread }
  TCPPEDebugLoopThread = class(TThread)
  private
    FTarget: TCPPEDebugTarget;
    function IsTargetDll(const AEvent: TDebugEvent): Boolean;
  protected
    procedure Execute(); override;
  public
    constructor Create(const ATarget: TCPPEDebugTarget);
  end;

  { TCPPEDebugTarget }
  TCPPEDebugTarget = class(TCPDebugTarget)
  private
    FExePath: string;
    ...
    FStoppedReason: TCPDebugStopReason;
    ...
    FDebugLoopThread: TCPPEDebugLoopThread;
  ...
  public
    ...
    function WaitForStop(out AEvent: TCPDebugStopEvent): Boolean; override;
...
constructor TCPDebugTarget.Create();
begin
  inherited Create();
  FBreakOnExceptions := False;
end;

destructor TCPDebugTarget.Destroy();
begin
  inherited Destroy();
end;

function TCPDebugTarget.ReadBytes(const AAddress: UInt64;
...
procedure TCPDebugTarget.SetRepatchOffset(const AOffset: Int64);
begin
  // Default no-op
end;

function TCPDebugTarget.GetRepatchOffset(): Int64;
begin
  Result := -1;
end;

function TCPDebugTarget.GetProcessHandle(): THandle;
begin
  Result := 0;
end;

function TCPDebugTarget.GetThreadHandle(): THandle;
begin
  Result := 0;
end;

procedure TCPDebugTarget.SignalConfigDone();
begin
  // Default no-op
end;

procedure TCPDebugTarget.WaitUntilReady();
begin
  // Default no-op
end;

procedure TCPDebugTarget.UnblockWaitForStop();
begin
  // Default no-op
end;

procedure TCPDebugTarget.SetBreakOnExceptions(const AEnabled: Boolean);
begin
  FBreakOnExceptions := AEnabled;
end;

function TCPDebugTarget.GetBreakOnExceptions(): Boolean;
begin
...
constructor TCPPEDebugLoopThread.Create(const ATarget: TCPPEDebugTarget);
begin
...
function TCPPEDebugLoopThread.IsTargetDll(
...
procedure TCPPEDebugLoopThread.Execute();
var
  LEvent: TDebugEvent;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            FTarget.WriteByte(UInt64(LRepatch), CP_DBG_INT3_OPCODE);
            ...
            LContext.ContextFlags := CONTEXT_FULL;
            GetThreadContext(FTarget.FMainThreadHandle, LContext);
            LContext.EFlags := LContext.EFlags and (not CP_DBG_TRAP_FLAG);
            SetThreadContext(FTarget.FMainThreadHandle, LContext);
...
constructor TCPPEDebugTarget.Create();
begin
  inherited Create();
  FExePath := '';
  FProcessHandle := 0;
...
destructor TCPPEDebugTarget.Destroy();
begin
  Stop();
  inherited Destroy();
end;

procedure TCPPEDebugTarget.SetExePath(const APath: string);
begin
  FExePath := APath;
end;

procedure TCPPEDebugTarget.SetDllMode(const ADllPath: string;
...
procedure TCPPEDebugTarget.ReadTextSectionFromPE();
var
...
function TCPPEDebugTarget.Start(): Boolean;
begin
  Result := False;
  ...
  FDebugLoopThread := TCPPEDebugLoopThread.Create(Self);
...
procedure TCPPEDebugTarget.Stop();
begin
...
function TCPPEDebugTarget.IsRunning(): Boolean;
begin
  Result := FStarted;
end;

function TCPPEDebugTarget.WaitForStop(out AEvent: TCPDebugStopEvent): Boolean;
begin
...
procedure TCPPEDebugTarget.Resume();
begin
  SetEvent(FResumeEvent);
end;

procedure TCPPEDebugTarget.SetTrapFlag();
begin
  FCapturedContext.EFlags := FCapturedContext.EFlags or CP_DBG_TRAP_FLAG;
end;

procedure TCPPEDebugTarget.ClearTrapFlag();
begin
  FCapturedContext.EFlags := FCapturedContext.EFlags and (not CP_DBG_TRAP_FLAG);
end;

function TCPPEDebugTarget.ReadByte(const AAddress: UInt64): Byte;
var
  LBytesRead: NativeUInt;
begin
  Result := 0;
...
procedure TCPPEDebugTarget.WriteByte(const AAddress: UInt64;
  const AValue: Byte);
var
...
function TCPPEDebugTarget.ReadUInt64(const AAddress: UInt64): UInt64;
var
  LBytesRead: NativeUInt;
begin
  Result := 0;
...
procedure TCPPEDebugTarget.FlushCode(const AAddress: UInt64;
  const ASize: Cardinal);
begin
  FlushInstructionCache(FProcessHandle, Pointer(AAddress), ASize);
end;

function TCPPEDebugTarget.GetContext(): TContext;
begin
  Result := FCapturedContext;
end;

procedure TCPPEDebugTarget.SetContext(const AContext: TContext);
begin
  FCapturedContext := AContext;
end;

function TCPPEDebugTarget.IsOurCode(const AAddress: UInt64): Boolean;
var
  LTextStart: UInt64;
begin
...
function TCPPEDebugTarget.GetProcessHandle(): THandle;
begin
  Result := FProcessHandle;
end;

function TCPPEDebugTarget.GetThreadHandle(): THandle;
begin
  Result := FMainThreadHandle;
end;

procedure TCPPEDebugTarget.SetRepatchOffset(const AOffset: Int64);
begin
  FRepatchOffset := AOffset;
end;

function TCPPEDebugTarget.GetRepatchOffset(): Int64;
begin
  Result := FRepatchOffset;
end;

procedure TCPPEDebugTarget.SignalConfigDone();
begin
  if FConfigDoneEvent <> 0 then
    SetEvent(FConfigDoneEvent);
end;

procedure TCPPEDebugTarget.WaitUntilReady();
begin
...
procedure TCPPEDebugTarget.UnblockWaitForStop();
begin
  if FStoppedEvent <> 0 then
