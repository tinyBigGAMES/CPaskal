...
type
  ...
  { TCopyDllEntry }
  TCopyDllEntry = record
    Target: TCPTargetPlatform;
    Path: string;
  end;

  { TLinkLibEntry }
  TLinkLibEntry = record
    Target: TCPTargetPlatform;
    Path: string;
  end;
  ...
  private
    ...
    FCopyDlls: TList<TCopyDllEntry>;
    FLinkLibs: TList<TLinkLibEntry>;
    FDllNames: TList<TDllNameEntry>;
    ...
    function TargetDefineName(const ATarget: TCPTargetPlatform): string;
  ...
  public
    ...
    procedure SetDllName(const ATarget: TCPTargetPlatform;
    ...
    procedure AddCopyDll(const ATarget: TCPTargetPlatform; const APath: string);
    procedure AddLinkLibrary(const ATarget: TCPTargetPlatform; const APath: string);
...
begin
  ...
  FCopyDlls := TList<TCopyDllEntry>.Create();
  FLinkLibs := TList<TLinkLibEntry>.Create();
  FDllNames := TList<TDllNameEntry>.Create();
...
function TCImporter.Peek(): TCToken;
begin
...
function TCImporter.GetMyraFuncName(const AFuncName: string): string;
begin
...
function TCImporter.MapCTypeToMyra(const ACType: string; const AIsPointer: Boolean; const APtrDepth: Integer; const AIsConstTarget: Boolean): string;
var
  LBaseType: string;
  LI: Integer;
begin
...
begin
  ...
  // Check against CPaskal keywords
  if FCPLexer.IsKeyword(AName) then
    Result := AName + '_';
end;

function TCImporter.SanitizeParamName(const AName: string): string;
begin
  Result := AName;
...
var
  LTargets: TList<TCPTargetPlatform>;
  LTarget: TCPTargetPlatform;
  LEntry: TCopyDllEntry;
  LLinkEntry: TLinkLibEntry;
  LI: Integer;
begin
  FOutput.Clear();
      ...
      begin
        ...
        begin
          if LLinkEntry.Target = LTarget then
            EmitFmt('@addlinklibrary "%s";', [LLinkEntry.Path]);
        end;
        Dec(FIndent);
      end;
...
procedure TCImporter.AddFunctionRename(const AOriginalName: string; const AMyraName: string);
begin
...
procedure TCImporter.AddCopyDll(const ATarget: TCPTargetPlatform; const APath: string);
var
  LEntry: TCopyDllEntry;
begin
...
{ TCImporter.AddLinkLibrary }
procedure TCImporter.AddLinkLibrary(const ATarget: TCPTargetPlatform;
  const APath: string);
var
  LEntry: TLinkLibEntry;
begin
  LEntry.Target := ATarget;
  LEntry.Path := APath.Replace('\', '/');
...
procedure TCImporter.EmitTargetDllNameBlock();
var
  LI: Integer;
  LEntry: TDllNameEntry;
begin
...
function TCImporter.TargetDefineName(const ATarget: TCPTargetPlatform): string;
begin
  // Maps TTargetKind to the compiler's predefined conditional symbol
  case ATarget of
    tpX86_64_Windows: Result := 'TARGET_X86_64_WINDOWS';
    tpX86_64_Linux  : Result := 'TARGET_X86_64_LINUX';
  else
    Result := '';
  end;
end;
...
procedure TCImporter.SetDllName(const ATarget: TCPTargetPlatform;
  ...
  try
      ...
      begin
        ...
        begin
          if LItem.Get('target').AsString() = 'linux64' then
            SetDllName(tpX86_64_Linux, LItem.Get('dll_name').AsString())
          else
            SetDllName(tpX86_64_Windows, LItem.Get('dll_name').AsString());
        end;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          if LItem.Get('target').AsString() = 'linux64' then
            AddCopyDll(tpX86_64_Linux, LItem.Get('path').AsString())
          else
            AddCopyDll(tpX86_64_Windows, LItem.Get('path').AsString());
        end;
      end;
    end;
...
var
  ...
  LCopyEntry: TCopyDllEntry;
  LDllNameEntry: TDllNameEntry;
  LKey: string;
  LI: Integer;
begin
  Result := False;
  ...
  try
    ...
    begin
      ...
      begin
        if LDllNameEntry.Target = tpX86_64_Linux then
          LJson.BeginObject()
            .Add('target', 'linux64')
    ...
    begin
      ...
      begin
        if LCopyEntry.Target = tpX86_64_Linux then
          LJson.BeginObject()
            .Add('target', 'linux64')
